...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator)
        {
            ...
            $this->mailer->send( 
                $translator->email,
                $translator->name,
                $subject, 'emails.job-changed-translator-new-translator',
                $dataEmail);
...
//        }
    ...
    public function sendSessionStartRemindNotification( $user, $job, $language, $due, $duration )
    {
        ...
        if ( $this->bookingRepository->isNeedToSendPush($user->id) )
        {
            ...
            ->sendPushNotificationToSpecificUsers($users_array, $job->id,
            $data, $msg_text,
            $this->bookingRepository
    ...
    public function sendExpiredNotification( $job, $user )
    {
        ...
        if ( $this->isNeedToSendPush($user->id) )
        {
            ...
            (
                $users_array, $job->id, $data, $msg_text,
    ...
    private function sendNotificationChangePending( $user, $job, $language, $due, $duration )
    {
        ...
        if ( $job->customer_physical_type == 'yes' )
            $msg_text = array(
                ...
                . $language . ' kl '
                . $duration . ' den '
                . $due . '. Vänligen säkerställ att du är förberedd för den tiden. Tack!'
            );
        else
            $msg_text = array(
                "en" => 'Du har nu fått telefontolkningen för '
                . $language . ' kl ' . $duration
                . ' den ' . $due
    ...
    private function getUserTagsStringFromArray( $users )
    {
        ...
        foreach ($users as $oneUser)
        {
            ...
            $user_tags .= '{"key": "email", "relation": "=", "value": "'
        ...
        if ( !Job::isTranslatorAlreadyBooked( $job_id, $cuser->id, $job->due ) )
        {
            if ( $job->status == 'pending' && Job::insertTranslatorJobRel( $cuser->id, $job_id ) )
            {
                ...
                array(
                    ...
                    . $job->duration . 'min, '
                    . $job->due . ' har accepterats av en tolk.
                    Vänligen öppna appen för att se detaljer om tolken.'
                );
                if ( $this->isNeedToSendPush( $user->id ))
                {
                    $users_array = array( $user );
                ...
                $language = TeHelper::fetchLanguageFromJobId( $job->from_language_id );
                $response['status'] = 'fail';
                $response['message'] = 'Denna '
                . $language . 'tolkning '
                . $job->duration . 'min '
                . $job->due . ' har redan accepterats av annan tolk.
        ...
        if ($cuser->is('customer'))
        {
            ...
            if ( $translator )
            {
                ...
                if ( $this->isNeedToSendPush( $translator->id ))
                {
                    ...
                    $this->sendPushNotificationToSpecificUsers(
                    $users_array, $job_id, $data, $msg_text,
            ...
            if ( $job->due->diffInHours( Carbon::now()) > 24 )
            {
                ...
                if ( $customer )
                {
                    ...
                    (
                        ...
                        . 'tolk, ' . $job->duration
                        . 'min ' . $job->due
                        . ', har avbokat tolkningen.
                        Vi letar nu efter en ny tolk som kan ersätta denne. Tack.'
                    );
...
//                Event::fire(new JobWasCanceled($job));
            ...
            {
                $response['status'] = 'fail';
                $response['message'] = 'Du kan inte avboka en bokning som sker
                inom 24 timmar genom DigitalTolk. Vänligen ring på +46 73 75 
                86 865 och gör din avbokning over telefon. Tack!';
            }
        }
        return $response;
    }
